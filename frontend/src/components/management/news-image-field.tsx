...
type NewsImageFieldProps = {
  name?: string;
  defaultValue?: string;
};

export function NewsImageField({ name = "image_url", defaultValue = "" }: NewsImageFieldProps) {
  const inputId = useId();
  ...
  return (
    <div className="space-y-2">
      <label className="block text-sm" htmlFor={inputId}>
        <span className="mb-1 block font-medium">Обложка (URL)</span>
        <input
          id={inputId}
          name={name}
          value={value}
          ...
          placeholder="/images/hero/tikhvin-monastery.webp"
        />
      </label>
