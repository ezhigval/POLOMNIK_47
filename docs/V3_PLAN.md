...
**Сделано в коде (identity):** таблица `user_identities`, OAuth-логин через identity, привязка при сессии (`Authorization: Bearer` на внутреннем `POST /auth/oauth`) и слияние в текущего пользователя (заявки, избранное, треды поддержки, назначения ролей). Конфликт имени/почты/телефона не перезаписывается. Пассажиров ещё нет. СНИЛС нет.

**Сделано в коде (профиль):** `PATCH /me` — имя, почта, телефон текущего кабинета. Смена телефона при включённом callcheck — тот же звонок, что при регистрации. Пароль через существующее восстановление. Пассажиры — следующий PR.
