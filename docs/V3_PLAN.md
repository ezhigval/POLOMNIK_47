...
**Сделано в коде (меню/дашборд):** пункты админки, карточки обзора, быстрые действия и блоки «Настройки»/«Синхронизация» показываются только при соответствующем permission. Карточка «Визиты» — при `view_stats`, цифры визитов не рисуются, только ссылка на Метрику. API по-прежнему проверяет те же права. Шаблоны ролей, фильтры заявок и CSV — отдельные PR.
