...
### v4 этапы 9–13 (продукт + инфра)

Этапы **9–11, 13** — код на `main`; goose на проде после деплоя: **00028** (`news_likes`, `news_comments`, goose **00027** `tour_views`).
...
| 12 | GitHub Actions deploy после CI | `DEPLOY_SSH_KEY` (+ опц. `DEPLOY_REMOTE`, `DEPLOY_DIR`) |
| 13 | Флаги туров в админке и на витрине | не назначать цену регулярному туру |
...
- [ ] Вопрос: сумма заявки на регулярный тур сейчас **0** — какая внутренняя цена? На витрине цены нет (этап 13).
...
| Деплой после зелёного CI на `main` | GitHub Actions (если `DEPLOY_SSH_KEY`) |
...
- Сумма заявки на **регулярный** тур сейчас **0** — какая внутренняя цена? На витрине цены и дат нет (этап 13).
- Статусы **`AWAITING_PAYMENT` / `PAID`** — нужны ли для эквайринга?
- Модерация комментариев новостей — пока нет; включать только по просьбе.
- Этапы **2–8** v4 (долг, live-адаптеры, оплата, юридичка, ИИ) — не блокируют выкладку 9–13.
