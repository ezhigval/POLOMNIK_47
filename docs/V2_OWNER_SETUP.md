...
- Админка: настройки сайта, получатели уведомлений (канал+адрес), роли (хеш пароля, назначение по UUID из `/account`)
- Полный админ = только `ADMIN_TOKEN` из env
- Телефон: sms.ru **callcheck** (не SMS)
- Соцвход: Яндекс / VK / Max / Telegram Login — кнопки; без env → «Пока что недоступно…»
- Google в UI скрыт
- Mailer: порт + SMTP; без `MAIL_ADAPTER=smtp` регистрация жива, письмо не шлётся
...
5. Без SMTP: регистрация работает, письмо подтверждения не уходит (это норма).
...
4. Smoke: новая заявка → сообщение; сообщение в поддержку → сообщение.
