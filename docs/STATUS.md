...
Продукт после этапа 1: **9** сессия/cookies — **готово** (`palomnik_viewed_tours`, блок «Вы смотрели»); **10** компактный список новостей на главной (не magazine 1+2) + SEO pass и семантика Wordstat без выдуманных текстов; **hero cleanup** (без CTA-кнопок и stat-плашек в шапке, цифры в «О службе») — **в коде**. **11** лента `/news` ≈ VK (лайки без регистрации, комментарии с кабинетом, без репоста; URL в тексте кликабельны с подсветкой; фото на весь экран). **12** автодеплой с `main` (**готово**, нужен `DEPLOY_SSH_KEY`). **13** флаги туров — **готово в коде**. Вебмастер / Search Console / Wordstat — клики владельца.
