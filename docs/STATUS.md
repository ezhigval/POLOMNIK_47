...
- регулярные туры: флаг `is_regular`, без цены и дат на витрине, заявка с суммой 0 (#32, goose **24**). Существующие туры (Успение и др.) **не** помечены регулярными. Внутреннюю цену не выдумывали. Уточнение владельца (v4 этап 13): регулярный = тур как факт, выезд не запланирован; популярный = `is_hot` (тег + подсветка); овербукинг оставляем.
...
Продукт после этапа 1 (**не начат**, код ленты не писать): **9** сессия/cookies (тот же кабинет, просмотренные туры), **10** компактный список новостей на главной (не magazine 1+2) + SEO pass и семантика Wordstat без выдуманных текстов, **11** лента `/news` ≈ VK (лайки без регистрации, комментарии с кабинетом, без репоста; URL в тексте кликабельны с подсветкой; фото на весь экран). **12** автодеплой с `main` (**готово**, нужен `DEPLOY_SSH_KEY`). **13** флаги туров — **готово в коде**: админка «Активный» / «Популярный» / «Овербукинг»; на сайте тег «Популярный», подсветка `is_hot`, регулярный без цены/дат, заявка при нуле мест если овербукинг. Вебмастер / Search Console / Wordstat — клики владельца.
