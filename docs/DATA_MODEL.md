...
Привязка соцсети при открытой сессии сливает другой кабинет **в текущего**: заявки (`bookings.user_id`), избранное, треды поддержки (открытый тред один: сообщения переносятся), назначения админ-ролей, пассажиры (`passengers.user_id`). Имя/почта/телефон целевого профиля не перезаписываются, если оба заполнены и различаются. Кабинет может править свои `name` / `email` / `phone` через `PATCH /me`.

Пассажиры кабинета (без СНИЛС):
...
passengers
  ...
  phone TEXT
  birth_date DATE
  passport TEXT       # как ввёл пользователь, без выдуманного формата
  created_at TIMESTAMPTZ
  updated_at TIMESTAMPTZ
```
