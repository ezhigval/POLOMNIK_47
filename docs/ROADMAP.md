...
Прод: https://tikhvin-palomnik.ru · теги `v1.0.0` … `v1.2.x` / `v2.0.0`.
...
- [x] Виджет «популярные направления»: данные из API популярных туров.
- [x] Отзывы: только из API, без выдуманных цитат.
...
- [x] Порт `Mailer` + SMTP-адаптер + `noop` (гексагон).
...
- [x] Исходящие: письма подтверждения регистрации с `info@`, когда `MAIL_ADAPTER=smtp`.
- [x] Без SMTP — регистрация жива; письмо просто не уходит.

### II.3 Регистрация и соцвход

- [ ] Имя + пароль + email и/или телефон (уже база).
- [x] Соцвход адаптерами: **Яндекс, Telegram Login, VK, Max** — кнопки; без env → «Пока что недоступно…»
- [ ] Один бот: login widget на том же `TELEGRAM_BOT_TOKEN` / domain в BotFather.
- [x] `GET /auth/methods` для UI (phone + social + mail).
