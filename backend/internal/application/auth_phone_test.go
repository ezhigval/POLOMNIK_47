...
func TestAuthMethodsPhoneUnavailable(t *testing.T) {
	...
	svc := application.NewAuthService(store, store, &fakePhone{available: false}, nil, application.SocialAuthConfig{}, "secret", time.Hour)
...
func TestRegisterRequiresConfirmedCallWhenPhoneAvailable(t *testing.T) {
	...
	svc := application.NewAuthService(store, store, phones, nil, application.SocialAuthConfig{}, "secret", time.Hour)
...
func TestRegisterWithoutPhoneAdapter(t *testing.T) {
	store := memory.NewStore()
	svc := application.NewAuthService(store, store, nil, nil, application.SocialAuthConfig{}, "secret", time.Hour)
	_, err := svc.Register(context.Background(), application.RegisterInput{
		Name:     "Иван Тест",
		Phone:    "+79001234567",
		Password: "password1",
