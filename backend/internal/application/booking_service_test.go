...
func TestBookingServiceCreateBookingWithNoopCRMIntegration(t *testing.T) {
	...
	service := NewBookingService(store, store, noop.NewCRMAdapter(), noop.NewAccountingAdapter(), notificationnoop.New())
...
func newBookingService(store *memory.Store) *BookingService {
	return NewBookingService(store, store, noop.NewCRMAdapter(), noop.NewAccountingAdapter(), notificationnoop.New())
}
