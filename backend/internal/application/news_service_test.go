...
func TestNewsServiceSetNewsPinnedAssignsSortOrderAndEnforcesLimit(t *testing.T) {
	store := memory.NewStore()
	svc := NewNewsService(store, nil)
	ctx := context.Background()
	day := time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)

	articles := make([]domain.NewsArticle, 0, 4)
	for i := 0; i < 4; i++ {
		article, err := svc.CreateNews(ctx, NewsArticleInput{
			Slug:        "star-" + string(rune('a'+i)),
			Title:       "Статья",
			Excerpt:     "Анонс",
			Body:        "Текст статьи.",
			PublishedAt: day.AddDate(0, 0, i),
	...
	for i := 0; i < domain.MaxPinnedNews; i++ {
		pinned, err := svc.SetNewsPinned(ctx, articles[i].ID, true)
		if err != nil {
			t.Fatalf("pin %d: %v", i, err)
		}
		if !pinned.IsPinned || pinned.SortOrder != i+1 {
			t.Fatalf("pin %d: expected sort_order %d, got %+v", i, i+1, pinned)
		}
	}

	_, err := svc.SetNewsPinned(ctx, articles[3].ID, true)
	if !errors.Is(err, domain.ErrTooManyPinnedNews) {
		t.Fatalf("expected too many pinned, got %v", err)
	}

	unpinned, err := svc.SetNewsPinned(ctx, articles[0].ID, false)
	if err != nil {
		t.Fatalf("unpin: %v", err)
	}
	if unpinned.IsPinned {
		t.Fatalf("expected unpinned, got %+v", unpinned)
	}

	reassigned, err := svc.SetNewsPinned(ctx, articles[3].ID, true)
	if err != nil {
		t.Fatalf("pin after unpin: %v", err)
	}
	if !reassigned.IsPinned || reassigned.SortOrder != 1 {
		t.Fatalf("expected next free sort_order 1, got %+v", reassigned)
	}
}
