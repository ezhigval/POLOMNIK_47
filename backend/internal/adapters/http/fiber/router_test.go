...
func TestManagementPinEndpointRejectsFourth(t *testing.T) {
	store := memory.NewStore()
	app := newTestAppWithStore(store, "secret-token")
	...
	pinReq := httptest.NewRequest(http.MethodPatch, "/api/v1/management/news/"+fourthID+"/pin", bytes.NewBufferString(`{"is_pinned": true}`))
	pinReq.Header.Set("Content-Type", "application/json")
	pinReq.Header.Set("X-Admin-Token", "secret-token")
	pinResp, err := app.Test(pinReq)
	if err != nil {
		t.Fatalf("pin: %v", err)
	}
	if pinResp.StatusCode != http.StatusUnprocessableEntity {
		body, _ := io.ReadAll(pinResp.Body)
		t.Fatalf("expected 422, got %d %s", pinResp.StatusCode, body)
	}
	var errBody struct {
		Error struct {
			Code    string `json:"code"`
	...
	if err := json.NewDecoder(pinResp.Body).Decode(&errBody); err != nil {
		t.Fatalf("decode: %v", err)
	}
	if errBody.Error.Code != "TOO_MANY_PINNED_NEWS" {
		t.Fatalf("expected TOO_MANY_PINNED_NEWS, got %+v", errBody)
